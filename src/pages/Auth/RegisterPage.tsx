...
import { signUp, updateProfile } from "@/services/auth.service";
...
function PasswordStrength({ password }: { password: string }) {
  ...
  const colors = ["bg-danger-500", "bg-danger-500", "bg-warning-500", "bg-success-500"];
      ...
      <ul className="space-y-1">
        {checks.map((c) => (
          <li key={c.label} className="flex items-center gap-1.5 text-xs">
            {c.ok ? <Check className="h-3 w-3 text-success-500" /> : <X className="h-3 w-3 text-slate-300" />}
...
export function RegisterPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (!authData.user) throw new Error("Compte non créé — réessayez");

      // Vérifier qu'on a bien une session active (confirmation email désactivée)
      if (!authData.session) {
        throw new Error("Vérifiez votre email pour confirmer votre compte, puis connectez-vous.");
      }

      const userId = authData.user.id;

      const company = await createCompany(companyName, companyLocation, companyLat, companyLng, userId);
          ...
          <div className="mt-10 space-y-4">
            {leftContent.items.map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary-500 text-xs font-bold text-white">{i + 1}</div>
      ...
      <div className="flex w-full items-center justify-center bg-slate-50 px-6 py-12 lg:w-1/2">
        <div className="w-full max-w-md">
          <div className="mb-8 flex gap-2">
            {["identity", "choice", step === "create" ? "create" : "join"].map((s, i) => {
              const steps = ["identity", "choice", step === "create" ? "create" : "join"];
              const current = steps.indexOf(step);
              return <div key={s} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= current ? "bg-primary-500" : "bg-slate-200"}`} />;
            })}
          </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <Input label="Prénom" placeholder="Awa" value={firstname} onChange={(e) => setFirstname(e.target.value)} />
                    <Input label="Nom" placeholder="Koné" value={lastname} onChange={(e) => setLastname(e.target.value)} />
                  </div>
