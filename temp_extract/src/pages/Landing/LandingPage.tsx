        ...
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
              <div className="rounded-3xl bg-slate-900 p-1 shadow-2xl">
                <div className="rounded-[22px] bg-slate-50 p-5">
                  ...
                  <div className="mb-4 grid grid-cols-2 gap-2.5">
                    {[
                      { label: "Employés", value: "24", color: "text-slate-900" },
                      { label: "Présents", value: "22", color: "text-success-600" },
                      { label: "Absents", value: "1", color: "text-danger-600" },
                      { label: "Retards", value: "1", color: "text-warning-600" },
                    ].map((k) => (
                  ...
                  <div className="rounded-xl bg-white p-3 shadow-sm">
                    <p className="mb-2 text-[10px] font-semibold text-slate-400">ACTIVITÉ EN DIRECT</p>
                    <div className="space-y-2">
                      {[
                        { dot: "bg-success-500", name: "Jean Dupont", status: "Présence validée" },
                        { dot: "bg-warning-500", name: "Paul Koffi", status: "Retard détecté" },
                        { dot: "bg-danger-500", name: "Marie Yao", status: "Absente" },
                      ].map((item) => (
                        <div key={item.name} className="flex items-center gap-2">
                          <div className={`h-2 w-2 shrink-0 rounded-full ${item.dot}`} />
                          <span className="text-xs text-slate-700">{item.name}</span>
                          <span className="ml-auto text-[10px] text-slate-400">{item.status}</span>
                        </div>
                      ))}
                    </div>
                  </div>
